...
def get_checkpoints_list_and_logs_path(args, cfg):
    ...
    if args.exp_path:
        ...
        # print(str(checkpoints_list[0].stem))
        # name = utils.find_checkpoint(args.model_dir, args.checkpoint)
        logs_path = os.path.join(args.logs_path, 'others', args.model_dir.split('/')[-1] + '-' + checkpoints_list[0].stem)
        logs_path = Path(logs_path)
        print(f'evaluation_dir:{logs_path}')
