...
class ClorePredictor(object):
    ...
    def get_prediction(self, clicker, prev_mask=None):
        ...
        print(f'{input_image.shape}prev_mask.shape:{prev_mask.shape[-2],prev_mask.shape[-1]},global_roi:{global_roi}')
        pred_logits, feature= self._get_prediction(image_nd, clicks_lists, is_image_changed)
        prediction = F.interpolate(pred_logits, mode='bilinear', align_corners=True,
                                   size=image_nd.size()[2:])
                         
        # print(f'95 prediction.unique:{torch.unique(prediction)}')
        for t in reversed(self.transforms):
            prediction = t.inv_transform(prediction)
        # print(f'98 prediction.unique:{torch.unique(prediction)}')
        ...
        # for vis
        self.coarse_pred = coarse_mask.cpu().numpy()[0, 0]

        # prev_mask = prev_mask
        clicks_list = clicker.get_clicks()
        image_full = self.original_image
        
        # 从[batch_size, channels, height, width]形状得到二维np数组
        coarse_mask_np = coarse_mask.cpu().numpy()[0, 0] 
        prev_mask_np = prev_mask.cpu().numpy()[0, 0] 

        if len(clicks_list) > 5:
        # if len(clicks_list) > 5:
            ...
            # y1,y2,x1,x2 = get_focus_cropv2(coarse_mask_np,prev_mask_np, global_roi,last_y,last_x, self.focus_crop_r)
            ...
            if (y2 < self.global_roi[1] or x2 < self.global_roi[3]) and (y1 != y2 and x1 != x2):
                ...
                # 这里传入的input_image是已经拼接过prev_mask的
                focus_pred = self._get_refine(pred_logits,input_image,clicks_list, feature, focus_roi, focus_roi_in_global_roi)
                ...
                # print(f'130 coarse_mask.unique:{torch.unique(coarse_mask)}')
                coarse_mask[:,:,y1:y2,x1:x2] = focus_pred  # paste local refinement to the whole mask
                coarse_mask = torch.sigmoid(coarse_mask)
                # print(f'133 coarse_mask.unique:{torch.unique(coarse_mask)}')
    ...
    def _get_prediction(self, image_nd, clicks_lists, is_image_changed):
        ...
        print(f'image_nd.shape:{image_nd.shape}, points_nd.shape:{points_nd.shape}')
    ...
    def _get_refine(self, coarse_mask, image, clicks, feature, focus_roi, focus_roi_in_global_roi):
        ...
        print(f'focus_roi:{focus_roi}')
        image_focus = image[:,:,y1:y2+1,x1:x2+1]
        print(f'image_focus.shape:{image_focus.shape}')
        ...
        print(f'image_focus.shape:{image_focus.shape}, points_nd.shape:{points_nd.shape}')
        ...
        output = self.net(image_focus, points_nd)

        cropped_logits = roi_align(coarse_mask,roi,image_focus.size()[2:], spatial_scale=1, aligned = True)
        self.focus_coarse = torch.sigmoid(cropped_logits).cpu().numpy()[0, 0] * 255
        self.focus_refined = torch.sigmoid(output['instances']).cpu().numpy()[0, 0] * 255
