...
from isegm.data.datasets import GrabCutDataset, BerkeleyDataset, DavisDataset, SBDEvaluationDataset, PascalVocDataset, Davis585Dataset, COCOMValDataset, GlasDataset
...
def get_dataset(dataset_name, cfg):
    if dataset_name == 'GrabCut':
    ...
    elif dataset_name == 'Glas' or dataset_name == 'glas':
        dataset =  GlasDataset(cfg.GLAS_PATH, csv='test.csv')
    elif cfg.ALL:
        ...
        if dataset_name == 'bns':
            ...
            name = 'MoNuSeg'
        elif dataset_name == 'nucls':
            name = 'NUCLS'
        elif dataset_name == 'glas':
            name = 'Glas'
        elif dataset_name == 'bloodcell':
            name = 'BloodCell'
        elif dataset_name == 'digestpath':
            name = 'DigestPath'
            ...
            dataset = None
            return dataset
        dataset = AllforInferenceDataset(cfg.ALL_PATH, csv='unique_all_test.csv', dataset_name=name)
    else:
        dataset = None
    return dataset
...
def compute_noc_metric(all_ious, iou_thrs, max_clicks=20):
    ...
    for iou_thr in iou_thrs:
        scores_arr = np.array([_get_noc(iou_arr, iou_thr)
                               for iou_arr in all_ious], dtype=np.int32)  # np.int
