...
def evaluate_dataset(dataset, predictor, vis = True, vis_path = './experiments/vis_val/',**kwargs):
    all_ious = []
    if vis:
        save_dir =  vis_path + dataset.dataset_name + '/'
...
def evaluate_sample(image, gt_mask, init_mask, predictor, max_iou_thr,
    ...
    with torch.no_grad():
        ...
        if init_mask is not None:
            ...
            # num_pm = 5
            num_pm = 8
            # num_pm = 2 # start Progressive_Merge from the 2nd click
        ...
        for click_indx in range(max_clicks):
            ...
            if progressive_mode:
                # 获取当前点击数量，看是否大于等于num_pm
                clicks = clicker.get_clicks()
                if len(clicks) >= num_pm: 
                    last_click = clicks[-1]
                    last_y, last_x = last_click.coords[0], last_click.coords[1]
                    pred_mask = Progressive_Merge(pred_mask, prev_mask,last_y, last_x)  # only refine the area around the last click
        ...
        if vis:
            if predictor.focus_roi is not None:
                ...
                pred_coarse = predictor.coarse_pred
                pred_coarse = pred_coarse > pred_thr


                out_image, focus_image = vis_result_refine(image, pred_mask,pred_coarse, gt_mask, init_mask, iou,click_indx+1,clicks_list,focus_roi, global_roi, vis_pred, last_y, last_x, focus_refined, focus_coarse)
...
# def vis_result_refine(image, pred_mask,pred_coarse, instances_mask, init_mask,  iou, num_clicks,  clicks_list, focus_roi, global_roi, prev_prediction, last_y, last_x, focus_refined, focus_coarse):
...
def vis_result_refine(image, pred_mask, pred_coarse, instances_mask, init_mask,  iou, num_clicks, clicks_list, focus_roi, global_roi, prev_prediction, last_y, last_x, focus_refined, focus_coarse):
    ...
    # 新增：对 pred_coarse 的处理（未经过改动的预测结果）
    pred_coarse = pred_coarse.astype(np.float32)
    pred_coarse_3 = np.repeat(pred_coarse[..., np.newaxis], 3, 2)
    color_mask_coarse = np.zeros_like(pred_coarse_3)
    color_mask_coarse[:, :, 0] = pred_coarse * 255
    fusion_coarse = image * 0.4 + color_mask_coarse * 0.6
    fusion_coarse = image * (1 - pred_coarse_3) + fusion_coarse * pred_coarse_3
    fusion_coarse = add_tag(fusion_coarse, 'pred coarse')

    color_mask_gt = np.zeros_like(pred_mask_3)
    color_mask_gt[:, :, 0] = instances_mask * 255

    color_mask_pred = np.zeros_like(pred_mask_3)
    color_mask_pred[:, :, 0] = pred_mask * 255

    color_mask_prev = np.zeros_like(prev_mask_3)
    color_mask_prev[:, :, 0] = prev_mask * 255
    ...
    if h < w:
        out_image = cv2.hconcat([image.astype(np.float32),
                                  fusion_init.astype(np.float32),
                                  fusion_pred.astype(np.float32),
                                  fusion_coarse.astype(np.float32),  # 新添加的pred_coarse区域
                                  fusion_gt.astype(np.float32),
                                  fusion_prev.astype(np.float32)])
    else:
        out_image = cv2.hconcat([image.astype(np.float32),
                                  fusion_init.astype(np.float32),
                                  fusion_pred.astype(np.float32),
