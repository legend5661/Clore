...
class ISTrainer(object):
    ...
    def batch_forward(self, batch_data, validation=False):
        ...
        with torch.set_grad_enabled(not validation):
            ...
            #     batch_data['images_focus'], batch_data['trimap_focus'], batch_data['instances_focus'])
            ...
            focus_net_input = torch.cat((images_focus, prev_output), dim=1) if self.net.with_prev_mask else images_focus
    ...
    def save_visualization(self, splitted_batch_data, outputs, refine_output, global_step, prefix):
        ...
        prev_masks_raw = prev_masks.copy()
        prev_masks = draw_probmap(prev_masks)

        # Full prediction processing
        predicted_instance_masks_raw = torch.sigmoid(outputs['instances'])[bindx,0].detach().cpu().numpy()
        predicted_instance_masks = draw_probmap(predicted_instance_masks_raw)
        
        # Full Pred overlay
        pred_full_color_mask = np.zeros_like(images)
        pred_full_color_mask[:,:,0] = (predicted_instance_masks_raw > 0.5) * 255
        pred_masked_full = 0.5 * images + 0.5 * pred_full_color_mask

        # Prev Full Pred overlay
        prev_color_mask = np.zeros_like(images)
        prev_color_mask[:,:,0] = (prev_masks_raw > 0.5) * 255
        prev_masked_full = 0.5 * images + 0.5 * prev_color_mask

        # Refine prediction processing
        ...
        pred_masked_full = add_tag(pred_masked_full, 'Full Pred Overlay')  # 新增标签
        prev_masks = add_tag(prev_masks, 'Prev Full Pred')
        prev_masked_full = add_tag(prev_masked_full, 'Prev Full Overlay')  # 新增标签
        ...
        pred_masked_image = add_tag(pred_masked_image, 'Refine Overlay')

        # 修改后的堆叠结构
        viz_image_full = np.hstack((image_with_points, 
                                  gt_masked_full,
                                  predicted_instance_masks,
                                  pred_masked_full,  # 新增图像
                                  prev_masks
                                  )).astype(np.uint8)
                                  
        viz_image_focus = np.hstack((images_focus_with_points, 
                                    gt_masked_focus, 
                                    predicted_mask_focus, 
                                    pred_masked_image,
                                    prev_masked_full  # 新增图像
                                    )).astype(np.uint8)
...
def get_three_focus(rois, images_focus, trimap_focus, instances_focus):
    ...
    for bindx in range(images_focus.shape[0]):
        ...
        tr_focus = tr_focus[0,0].cpu().detach().numpy() * 255
        ...
        # cv2.imwrite(output_images_path + '/' + 'tr_focus.jpg',
        #                 tr_focus, [cv2.IMWRITE_JPEG_QUALITY, 85])
        # cv2.imwrite(output_images_path + '/' + 'in_focus.jpg',
        #                 in_focus, [cv2.IMWRITE_JPEG_QUALITY, 85])
        # print('line 535 cv2.imwrite done!')
        # exit(0)
