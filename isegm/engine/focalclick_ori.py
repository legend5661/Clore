    ...
    def batch_forward(self, batch_data, validation=False):
        ...
        with torch.set_grad_enabled(not validation):
            ...
            with torch.no_grad():
                ...
                # 点击循环，进行点击次数次的前向，得到第一阶段的输出结果（添加n个点则进行n+1次前向，这里是前n次）
                for click_indx in range(num_iters):
                    ...
                    # 提供一种为不同的点击使用不同模型的选项，一般情况下，使用self.net
            ...
            # output['feature']是第一阶段的特征图，output['instances']是第一阶段的输出结果
            full_feature, full_logits = output['feature'], output['instances']
            # 将rois切成rois.shape[0]个部分，bboxes是一个长度为rois.shape[0]的元组，每个元素是一个bbox
